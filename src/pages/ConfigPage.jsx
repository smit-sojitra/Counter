...
export default function ConfigPage({ setConfig }) {
    const [initial, setInitial] = useState(() => {
        // Get the saved initial value from localStorage, or use 0 as default
        const savedConfig = JSON.parse(localStorage.getItem('counterConfig'));
        return savedConfig ? savedConfig.initial : 0;
    });
    const [target, setTarget] = useState(() => {
        // Get the saved target value from localStorage, or use 100 as default
        const savedConfig = JSON.parse(localStorage.getItem('counterConfig'));
        return savedConfig ? savedConfig.target : 100;
    });
    const [interval, setInterval] = useState(() => {
        // Get the saved interval value from localStorage, or use 25 as default
        const savedConfig = JSON.parse(localStorage.getItem('counterConfig'));
        return savedConfig ? savedConfig.interval : 25;
    });
    ...
    useEffect(() => {
        ...
        const configData = { initial, target, interval };
        setConfig(configData);
        localStorage.setItem('counterConfig', JSON.stringify({ initial, target, interval }));
    }, [])
    const handleSubmit = () => {
        if (initial >= target) return setError('Initial must be less than target');
        if (interval <= 0 || target <= 0) return setError('Interval and target must be > 0');
        if (interval > target) return setError('Interval cannot exceed target');
        localStorage.removeItem('counterApp');
        // setConfig({ initial, target, interval });
        const configData = { initial, target, interval };
        setConfig(configData);
        localStorage.setItem('counterConfig', JSON.stringify({ initial, target, interval }));
    ...
    return (
        <div className="p-4 min-h-screen relative w-screen text-white bg-[#212121] gap-10 justify-center items-center flex flex-col">
            ...
            {/* <button className='button relative' onClick={handleSubmit}>Counter</button> */}
